---
title: 'Leicester SA: Using GP only for outcome ascertainment'
author: "Xiaonan Liu"
...
In training data (N=`r nrow(tot_train)`), `r sum(tot_train$SA_T2DM_status)` (`r pretty_dp(sum(tot_train$SA_T2DM_status)/nrow(tot_train),dp=1,pct=TRUE)`%) developed T2D cases at follow up (median follow-up is `r pretty_dp(median(tot_train$SA_T2DM_time),1)` years). There were `r tot_train%>%filter(SA_T2DM_time>10)%>%nrow()` had follow-up time exceeding 10 years, among which there were `r tot_train%>%filter(SA_T2DM_time>10)%>%pull(SA_T2DM_status)%>%sum()` cases.

In test data (N=`r nrow(tot_test)`), `r sum(tot_test$SA_T2DM_status)` (`r pretty_dp(sum(tot_test$SA_T2DM_status)/nrow(tot_test),dp=1,pct=TRUE)`%) developed T2D cases at follow up (median follow-up is `r pretty_dp(median(tot_test$SA_T2DM_time),1)` years). There were `r tot_test%>%filter(SA_T2DM_time>10)%>%nrow()` had follow-up time exceeding 10 years, among which there were `r tot_test%>%filter(SA_T2DM_time>10)%>%pull(SA_T2DM_status)%>%sum()` cases.
...
rs_train<-rs_train%>%
  ...
  mutate(T2D10_status=ifelse(SA_T2DM_status==1 & SA_T2DM_time<=10,1,0))
...
long_train<-rs_train%>%
  select(ID,SA_T2DM_status,T2D10_status,diab_rs,Eth_rs,PRS_rs)%>%
  pivot_longer(!c("ID","SA_T2DM_status","T2D10_status"), names_to = "Score", values_to = "Value")%>%
...
perc_nri_train_tb<-as.data.frame(perc_nri_train$nri)[1:3,]%>%
  mutate(NRI=c("Overall NRI","NRI for cases","NRI for controls"),
